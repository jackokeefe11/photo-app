...
class TestLikeCommentListEndpoint(unittest.TestCase):
    ...
    def test_like_comment_valid_request_201(self):
        comment_id = utils.get_comment_by_id(self.current_user.get('id'))
        url = '{0}/api/comments/likes'.format(root_url)
        response = requests.post(url, json={'comment_id': comment_id})
        # print(response.text)
        ...
        new_like_db = utils.get_comment_by_id(new_like.get('id'))
        ...
        self.assertEqual(utils.get_comment_by_id(new_like.get('id')), [])
